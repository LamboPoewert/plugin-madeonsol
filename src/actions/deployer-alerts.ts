...
export const deployerAlertsAction: Action = {
  ...
  handler: async (
    ...
    const data = result.data as { alerts: Array<{ title: string; token_symbol: string; priority: string; market_cap_at_alert: number | null; deployer_sol_balance: number | null; deployers: { tier: string; bonding_rate: number | null; runner_rate?: number | null; labeled_tokens?: number | null; avg_time_to_bond_minutes?: number | null }; kol_buys: { count: number; kols: string[] } | null }> };
